    ...
    data.getNotes = function (categoryName, next) {
        database.getDb(function (err, db) {
            if (err) {
                ...
                db.notes.findOne({ name : categoryName }, next); //need callback always
            }
        });
    };
    
    data.addNotes = function (categoryName, noteToInsert, next) {
        database.getDb(function (err, db) {
            if (err) {
                next(err);
            } else {
        ...
        database.getDb(function (err, db) {
            if (err) {
                ...
                db.notes.count(function (err, count) {
                    if (err) {
                        console.log("Failed with seed data: " + err);
                    } else {
                        if (count == 0) {
                            console.log("Seeding the database...");
                            seedData.initialNotes.forEach(function (item) {
                                db.notes.insert(item, function (err) {
                                    if (err) console.log("Failed to insert data: " + err);
                                })
                            })
                        } else {
